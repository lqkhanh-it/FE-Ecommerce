...
export default function DashboardProductEdit(props) {
  ...
  useEffect(() => {
    if (product) {
      ...
      axios
        .get(`https://be-ecommerce-year4.herokuapp.com/category`)
        .then((res) => {
          setCate(res.data);
  ...
  const onSubmit = (event) => {
    ...
    axios
      .post(
        `https://be-ecommerce-year4.herokuapp.com/products/update/${product._id}`,
        formData,
        config
      )
      .then(() => {
  ...
  const addNewCate = () => {
    axios.post("https://be-ecommerce-year4.herokuapp.com/category", {
